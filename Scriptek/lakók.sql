PROMPT Táborhely Üzemeltetõ: Lakók almenû
PROMPT
PROMPT ******************************************************************
PROMPT * Lakók listázása                     (Lakó_lista)               *
PROMPT * Új Lakó felvétele                   (Lakó_új)                  *
PROMPT * Lakó módosítása                     (Lakó_mód)                 *
PROMPT *                                                                *
PROMPT * Lakó törlése                        (Lakó_töröl)               *
PROMPT * Lakók meghatározott nem szerint     (Lakó_lista_nem)           *
PROMPT * Házba még be nem sorolt lakók       (Lakó_lista_háznélkül)     *
PROMPT * Résszót tartalmazó nevû lakók       (Lakó_lista_résznév)       *
PROMPT *                                                                *
PROMPT * Adott nemû Lakók átlagéletkora      (Lakó_nem_átlagéletkor)    *
PROMPT * A 18 évnél fiatalabb lakók száma    (Lakó_sum_tizennyolc_alatt)*
PROMPT *                                                                *
PROMPT *                                                                *
PROMPT * Azon lakók, a legnagyobb házakból    (Lakó_Legnagyobb_házak)   *
PROMPT * A legfiatalabb lakó                  (Lakó_Legfiatalabb_ALL)   *
PROMPT * A legfiatalabb lakó                  (Lakó_legfiatalabb_maxxal)*
PROMPT * A legidõsebb lakó                    (Lakó_legidõsebb_minnel)  *
PROMPT * A legidõsebb lakó                    (Lakó_legidõsebb_ANY)     *
PROMPT ******************************************************************

ACCEPT al PROMPT 'A kívánt almenû eléréséhez írjon be, egy a zárójelekben lévõ szót: '
@@ &al